...
from lobanalyzer.constants import (
    FeatureIndex, LABEL_DOWN, LABEL_STABLE, LABEL_UP,
...
class SignalAutocorrelation:
    ...
    Attributes:
        ...
        acf_values: ACF at each lag (ACF[0] = 1.0 always)
        lags: Lag values (0, 1, 2, ..., max_lag)
        half_life: Lag where ACF drops below 0.5
        decay_rate: Estimated exponential decay rate (higher = faster decay)
        persistence_interpretation: Human-readable interpretation
    """
    signal_name: str
    signal_index: int
...
class LeadLagRelation:
    ...
    Attributes:
        leader: Name of leading signal
        follower: Name of following signal
        leader_index: Index of leading signal
        follower_index: Index of following signal
        optimal_lag: Lag at max correlation (positive = leader leads by this many samples)
        max_correlation: Maximum cross-correlation value
        is_significant: True if |max_correlation| > 0.2
    """
    leader: str
    follower: str
    leader_index: int
...
class PredictiveDecay:
    ...
    Attributes:
        signal_name: Name of the signal
        signal_index: Index in feature array
        lags: Lag values tested
        correlations: Signal-label correlation at each lag
        half_life: Lag where |corr| drops to half of peak
...
class LevelVsChangeAnalysis:
    ...
    Attributes:
        signal_name: Name of the signal
        signal_index: Index in feature array
        level_correlation: Correlation of signal level with label
        change_correlation: Correlation of signal change with label
        level_auc: AUC for Up prediction using signal level
        change_auc: AUC for Up prediction using signal change
...
class TemporalDynamicsSummary:
    ...
    Attributes:
        autocorrelations: Autocorrelation analysis for each signal
        lead_lag_relations: Lead-lag relationships between signals
        predictive_decays: Predictive decay for each signal
        level_vs_change: Level vs change analysis for each signal
        optimal_lookback: Recommended lookback window for sequence models
        sequence_model_justified: Whether temporal structure justifies sequence model
...
def compute_autocorrelation(
    ...
    Returns:
        ...
        decay_rate: Estimated exponential decay rate (-ln(ACF[1]))
    
    Example:
        >>> signal = np.random.randn(1000)
        >>> acf, half_life, decay_rate = compute_autocorrelation(signal)
        >>> print(f"Half-life: {half_life} samples")
    """
    n = len(signal)
...
def compute_signal_autocorrelations(
    ...
    Args:
        features: (N, F) feature array (sample-level, NOT aligned)
...
def compute_lead_lag_relations(
    ...
    Returns:
        List of significant LeadLagRelation, sorted by |correlation|
    """
    if signal_indices is None:
        signal_indices = CORE_SIGNAL_INDICES
...
def compute_all_predictive_decays(
    ...
    Args:
        ...
        signal_indices: Which signals to analyze (default: CORE_SIGNAL_INDICES)
        lags: Lags to test (default: [0, 1, 2, 5, 10, 20, 50, 100])
...
def compute_all_level_vs_change(
    ...
    Returns:
        List of LevelVsChangeAnalysis for each signal
    """
    if signal_indices is None:
        signal_indices = CORE_SIGNAL_INDICES
