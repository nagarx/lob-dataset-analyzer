...
Reference:
    Standard time series analysis methods for categorical sequences.
"""
...
def run_label_analysis(
    ...
    """
    Run comprehensive label analysis on pre-loaded data.
    
    This is a convenience function that runs all label analysis components
    in one call. For large datasets, consider using the streaming functions
    from lobanalyzer.streaming instead.
    
    Args:
        features: Aligned feature array (N, 98) - features[i] corresponds to labels[i]
        labels: Label array (N,) with values in {-1, 0, 1}
    
    Returns:
        LabelAnalysisSummary with all analysis results:
            - distribution: Label counts and percentages
            - autocorrelation: Temporal dependencies
            - transition_matrix: State transition probabilities
            - regime_stats: Per-regime statistics
            - signal_correlations: Signal-label correlations
    ...
    Note:
        For large datasets (>1M samples), use the streaming functions instead:
        - lobanalyzer.streaming.compute_streaming_label_analysis()
    """
    # Run all analyses
    ...
    regime_stats = compute_regime_stats(features, labels)
    signal_correlations = compute_signal_label_correlations(features, labels)
